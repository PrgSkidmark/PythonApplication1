...
class particle_motion_none(particle_motion):
...
class particle_motion_twirl(particle_motion):
    def init_vector(self, particle):
...
class particle_motion_trajectory(particle_motion):
    ...
    def update(self, particle):
        SHOT_SPEED = 10
        #int(math.cos(particle.initial_angle) * SHOT_SPEED)
        particle.set_vector_x(10);
        particle.x += int(math.cos(particle.initial_angle) * SHOT_SPEED);

        theta = particle.initial_angle;
        g = -9.81;
        v = particle.power;
        y0 = particle.init_y;
        d = (particle.x+particle.get_vector_x());

        # note that we increase height by subtracting y and visa versa
        particle.y = int( y0 - (d * math.tan(theta) - (g * d**2) / (2 * v**2 * math.cos(theta)**2) ) );
        #particle.y = int( y0 - (d * math.tan(theta) + (g * d**2) / (2 * v**2 * math.cos(theta)**2) ) );
        #y0 -
        if (particle.y > particle.height):
...
class particle_motion_trajectorybak(particle_motion):
    def init_vector(self, particle):
        particle_vector_y = int(random.randint(75,120)/particle.size) #velocity
        particle.set_vector_x(0)
        particle.set_vector_y(particle_vector_y)
...
class particle_motion_wrapsides(particle_motion):
    def init_vector(self, particle):
...
class particle_motion_bouncesides(particle_motion):
    def init_vector(self, particle):
        particle_vector_x = random.randint(-2, 2)
        particle_vector_y = random.randint(-2, 2)
        while (particle_vector_x == 0):
...
class particle_motion_down_straight(particle_motion):
    def init_vector(self, particle):
        particle_vector_x = 0
        if (particle.size >= 2):
...
class particle_motion_down_zigzag(particle_motion):
    def init_vector(self, particle):
...
class particle_placement_insidecannon(particle_placement):
    ...
    def reset(self, *args):
        ...
        for particle in self.particle_array:
            particle.x = -4
            particle.y = int(self.surface_height/2)
